...
# print(f"\nUsing device: {DEVICE}\n") # Commenting out excessive prints
...
openai.api_key = os.getenv("OPENAI_API_KEY", "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")
...
try:
    ...
    # print("Text document loaded.") # Commenting out excessive prints
...
# print("Text splitter initialized.") # Commenting out excessive prints
...
# print("HuggingFace Embeddings initialized.") # Commenting out excessive prints

try:
    loader = PyPDFLoader(PDF_FILE_PATH)
    docs = loader.load()
    embeddings = hf.embed_documents([doc.page_content for doc in docs])
    # print("PDF loaded and embeddings created.") # Commenting out excessive prints
except Exception as e:
...
try:
    ...
    if not collection.get()['ids'] and docs: # Only add if collection is empty and docs are available
        # print("Populating ChromaDB collection...") # Commenting out excessive prints
        collection.add(
    ...
    # print("ChromaDB initialized and ready.") # Commenting out excessive prints
except Exception as e:
    print(f"Error setting up ChromaDB: {e}")
    db = None # Set to None to handle gracefully later

tokenizer = GPT2Tokenizer.from_pretrained("gpt2")
# print("GPT2 Tokenizer loaded.") # Commenting out excessive prints
...
class ChatBot_View(APIView):
    def post(self, request):
        ...
        if call_disconnect:
            ...
            try:
                ...
                if not conversation_df.empty:
                    ...
                    Your Response:
                    """
                    response_disposition = openai.ChatCompletion.create(
                        model="gpt-3.5-turbo",
                        messages=[
                    ...
                    if "Meeting Scheduled".lower() in disposition.lower():
                        ...
                        """
                        response_meeting_time = openai.ChatCompletion.create(
                            model="gpt-3.5-turbo",
                            messages=[
        ...
        if "hello" in question.lower() and conversation_history: # Check if conversation already exists
            ...
            current_day_str = current_datetime.strftime("%A") # Full weekday name
            ...
            1️⃣ Warm Introduction: Greet the customer ({current_time_str}), introduce yourself confidently, and directly state the fund’s name and its purpose in the first message.
            ...
            Time Validation Rules (Apply these strictly to the *proposed meeting time*):
            ...
            - If the **proposed meeting time is before current time on the same day**: "We can’t schedule meetings in the past. The earliest available time is {(current_datetime + timedelta(minutes=30)).strftime("%I:%M %p")} IST today."
            - If the **proposed meeting time is outside 9:00 AM–8:00 PM IST for any requested valid day**: "Please choose a time between 9:00 AM and 8:00 PM IST."
        ...
        try:
            ...
            if conn:
                with conn.cursor() as cur:
                    # Sanitize inputs using parameterized queries for safety
                    insert_query = """
